...
{
	@NonNls
	private static final String NONE = GwtBundle.message("gwt.package.web.facet.none");
	...
	public GwtFacetEditor(final FacetEditorContext editorContext, final FacetValidatorsManager validatorsManager, GwtFacetConfiguration configuration)
	{
		...
		myCompilerOutputDirField.addBrowseFolderListener(project, new CompilerOutputBrowseFolderActionListener(project, module, myCompilerOutputDirField));
	...
	private ValidationResult checkUserJarLibrary(final String sdkPath)
	{
		...
		final VirtualFile userJar = JarFileSystem.getInstance().findFileByPath(FileUtil.toSystemIndependentName(userJarPath) + JarFileSystem
				.JAR_SEPARATOR);
		ModuleRootModel rootModel = myEditorContext.getRootModel();
		if(rootModel == null || userJar == null)
		{
	...
	{
		return !myConfiguration.getGwtSdkPath().equals(myGwtPathEditor.getPath()) || myConfiguration.getOutputStyle() != getOutputStyle() ||
				myConfiguration.isRunGwtCompilerOnMake() != myRunGwtCompilerCheckbox.isSelected() || !Comparing.equal(myConfiguration.getWebFacetName(),
				getSelectedWebFacet()) || !Comparing.equal(myConfiguration.getCompilerOutputPath(), FileUtil.toSystemIndependentName(myCompilerOutputDirField
				.getText())) || !myConfiguration.getAdditionalCompilerParameters().equals(myAdditionalCompilerParametersField.getText()) || !String.valueOf
				(myConfiguration.getCompilerMaxHeapSize()).equals(myCompilerHeapSizeField.getText()) || isPackagingSettingsModified() || myTableView.isEditing();
	}
