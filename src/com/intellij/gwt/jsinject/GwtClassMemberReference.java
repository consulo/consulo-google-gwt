...
public class GwtClassMemberReference extends PsiReferenceBase<JSGwtReferenceExpressionImpl> {
  ...
  @NonNls private static final String NEW_EXPRESSION = "new";
  @NonNls private static Map<PsiType, String> ourPrimitiveTypes = new HashMap<PsiType, String>();
  static {
  ...
  private Map<String, PsiMember> getMembersMap(final @NotNull PsiClass aClass) {
    ...
    if (value == null) {
      value = getElement().getManager().getCachedValuesManager().createCachedValue(new CachedValueProvider<Map<String, PsiMember>>() {
  ...
  public Object[] getVariants() {
    PsiClass psiClass = resolveQualifier();
    if (psiClass == null) return ArrayUtil.EMPTY_OBJECT_ARRAY;

    Map<String, PsiMember> map = getMembersMap(psiClass);
    if (map == null) return ArrayUtil.EMPTY_OBJECT_ARRAY;

    List<LookupItem<PsiMember>> lookupItems = new ArrayList<LookupItem<PsiMember>>();
    for (Map.Entry<String, PsiMember> entry : map.entrySet()) {
      PsiMember member = entry.getValue();
      LookupItem<PsiMember> item = LookupElementFactoryImpl.getInstance().createLookupElement(member, entry.getKey());
  ...
  public PsiElement handleElementRename(String newElementName) throws IncorrectOperationException {
    ...
    ASTNode newNode = JSChangeUtil.createExpressionFromText(myElement.getProject(), newText, JavaScriptSupportLoader.GWT_DIALECT);
