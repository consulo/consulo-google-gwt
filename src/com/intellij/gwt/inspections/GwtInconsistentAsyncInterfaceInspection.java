...
public class GwtInconsistentAsyncInterfaceInspection extends BaseGwtInspection {
  ...
  @Nullable
  private static ProblemDescriptor[] checkAsyncServiceForRemote(PsiClass sync, PsiClass async, final GwtVersion gwtVersion,
    ...
    if (!methodsToCopy.isEmpty()) {
      ...
      problems.add(manager.createProblemDescriptor(getElementToHighlight(async), message, fixes,
                                                   ProblemHighlightType.GENERIC_ERROR_OR_WARNING));
    }
  ...
  private static class CopyMethodToSyncQuickFix extends BaseGwtLocalQuickFix {
    ...
    private CopyMethodToSyncQuickFix(final PsiMethod method, final PsiClass sync) {
      super(GwtBundle.message("quick.fix.name.create.sync.method.for.async",
                              PsiFormatUtil.formatMethod(method, PsiSubstitutor.EMPTY, PsiFormatUtil.SHOW_NAME | PsiFormatUtil.SHOW_PARAMETERS, PsiFormatUtil.SHOW_TYPE)));
      mySync = sync;
      myMethod = method;
    }
  ...
  private static class CopyMethodToAsyncQuickFix extends BaseGwtLocalQuickFix {
    ...
    private CopyMethodToAsyncQuickFix(final PsiClass async, final PsiMethod method, final GwtVersion gwtVersion) {
      super(GwtBundle.message("quick.fix.name.copy.method.to.asynch",
                              PsiFormatUtil.formatMethod(method, PsiSubstitutor.EMPTY, PsiFormatUtil.SHOW_NAME | PsiFormatUtil.SHOW_PARAMETERS, PsiFormatUtil.SHOW_TYPE)));
      myAsync = async;
      myMethod = method;
