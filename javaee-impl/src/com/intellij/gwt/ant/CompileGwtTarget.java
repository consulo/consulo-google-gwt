...
{
	private CompileGwtTarget(@NotNull JavaEEGoogleGwtModuleExtension facet, final String depends, final String description, final ModuleChunk chunk)
	{
		super(GwtBuildProperties.getCompileGwtTargetName(facet), depends, description, null);
		Project project = facet.getModule().getProject();

		MultiValuesMap<String, JavaEEGoogleGwtModuleExtension> paths = GwtBuildExtension.getGwtSdkPaths(project);
		...
		@NonNls String outputDir = facet.getCompilerOutputPath();
		if(StringUtil.isEmpty(outputDir))
		{
			outputDir = BuildProperties.propertyRef(BuildProperties.getModuleChunkBasedirProperty(chunk)) + "/GWTCompilerOutput" + BuildProperties
					.convertName(facet.getModule().getName());
		}
	...
	public static CompileGwtTarget create(@NotNull JavaEEGoogleGwtModuleExtension facet, final GenerationOptions genOptions, final ModuleChunk chunk)
	{
